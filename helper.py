...
def count_plot_total(df):
    ...
    df = df.reset_index()
    return px.bar(df, x='Status', y='Total', color='Status', color_discrete_sequence=['#DD4132',  '#FAE03C', '#79C753'])


def pie_chart_total(df):
    df = df.groupby('Status')[['Total']].sum()
    df = df.reset_index()
    return px.pie(data_frame=df, values='Total', names='Status', hole=0.5, color_discrete_sequence=['#DD4132', '#79C753', '#FAE03C'])


def area_scatter(df):
    fig1 = px.area(df, x='Date', y='Total', color='Status',
                   color_discrete_sequence=['#DD4132', '#79C753', '#FAE03C'])
...
def violin_plot_tot(df):
    ...
    fig.add_trace(go.Violin(y=x1, points='all',
                            box_visible=True, name='Confirmed', line_color='#DD4132'))
    fig.add_trace(go.Violin(y=x2, points='all',
                            box_visible=True, name='Recovered', line_color='#79C753'))
    fig.add_trace(go.Violin(y=x3, points='all',
                            box_visible=True, name='Deceased', line_color='#FAE03C'))
    return fig
...
def get_st(df):
    ...
    fig.add_trace(
        go.Bar(x=t1.State, y=t1.Count, name='Confirmed',
               textangle=0, marker_color='#DD4132'),
        row=1, col=1
    )
    fig.add_trace(
        go.Bar(x=t2.State, y=t2.Count, name='Recovered', marker_color='#79C753'),
        row=1, col=2
    )
    fig.add_trace(
        go.Bar(x=t3.State, y=t3.Count, name='Deceased', marker_color='#FAE03C'),
        row=1, col=3
    )
...
def pplott(df1, states, ch):
    ...
    f3.add_trace(go.Violin(y=x1, points='all',
                           box_visible=True, name='Confirmed', marker_color='#DD4132'))
    f3.add_trace(go.Violin(y=x2, points='all',
                           box_visible=True, name='Recovered', marker_color='#79C753'))
    f3.add_trace(go.Violin(y=x3, points='all',
                           box_visible=True, name='Deceased', marker_color='#FAE03C'))
